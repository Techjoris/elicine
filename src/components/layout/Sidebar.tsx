    ...
    <div className="flex flex-col justify-between h-full space-y-6">
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <button 
            ...
            title={t.closeSidebar || "Masquer la barre latérale"}
          >
        ...
        >
          ...
          <span>{t('nav.new_search') || t.newSearch}</span>
        </button>
        ...
        <div className="pt-4 border-t border-slate-200/80 dark:border-white/[0.08] space-y-2">
          <div className="flex items-center justify-between px-1">
            ...
            {searchHistory.length > 0 && (
              ...
              >
                {t('history.clear') || t.historyClear}
              </button>
            )}
          </div>
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
            {searchHistory.length === 0 ? (
              <p className="px-1 text-xs text-slate-400 dark:text-zinc-500 italic">
                {t('history.empty') || t.historyEmpty}
              </p>
            ) : (
              searchHistory.map((item) => {
                const cleanQuery = item.query.replace(/^#\s*/, '').trim();
      ...
      <div className="space-y-2.5 pt-3 border-t border-slate-200/80 dark:border-white/[0.08]">
        ...
        {isPro ? (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-amber-700 dark:text-amber-300">{t('pro.active') || t.proActive}</span>
            </div>
        ...
        <button
          ...
          title={t('nav.support') || t.supportProject}
        >
          <div className="flex items-center gap-2">
            <span className="text-sm">☕</span>
            <span>{t('nav.support') || t.supportProject}</span>
          </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-white/[0.03] hover:bg-slate-200 dark:hover:bg-white/[0.06] border border-slate-200 dark:border-white/[0.08] transition-all cursor-pointer select-none group"
          title={t('nav.settings') || t.settings}
          aria-label={t('nav.settings') || t.settings}
        >
          <div className="flex items-center gap-2">
            ...
            <span>{t('nav.settings') || t.settings}</span>
          </div>
          <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-bold bg-slate-200 dark:bg-white/[0.06] px-1.5 py-0.5 rounded">
            {t.settingsOptions || 'Options'}
          </span>
        </button>
        ...
        <div className="md:hidden flex items-center justify-between px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08]">
          <span className="text-xs font-semibold text-slate-700 dark:text-zinc-300">{t('theme.appearance') || t.appearance}</span>
          <button
            type="button"
            onClick={toggleTheme}
          ...
          >
            {effectiveTheme === 'dark' ? (
              <>
                ...
                <span>{t('theme.dark') || t.themeDark}</span>
              </>
            ) : (
              <>
                <Sun size={12} className="text-amber-500" />
                <span>{t('theme.light') || t.themeLight}</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className="md:hidden flex items-center justify-between px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08]">
          <span className="text-xs font-semibold text-slate-700 dark:text-zinc-300">{t.languageLabel || 'Langue'}</span>
          ...
          if (isConnected) {
            return (
              <div 
                ...
                title={t.manageProfile || 'Gérer mon profil'}
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-xs font-bold text-slate-900 dark:text-white truncate group-hover:text-slate-700 dark:group-hover:text-zinc-200 transition-colors">
                      {(activeUser as any)?.user_metadata?.full_name || activeUser.email || (activeUser as any)?.name || (t.defaultUserName || 'Cinéphile')}
                    </span>
          ...
          return (
            <div className="md:hidden pt-0.5">
              ...
              >
                ...
                <span>{t.loginBtn || 'Se connecter'}</span>
              </button>
            </div>
          );
        })()}
        ...
        {SHOW_DEV_PANEL && (
          ...
          >
            <span>🛠️</span>
            <span>{t.devConsole || 'Console Développeur'}</span>
        ...
        <div className="pt-2 text-center">
          ...
          >
            {t('nav.terms') || t.termsAndPrivacy || 'Conditions & Confidentialité'}
          </button>
        </div>
