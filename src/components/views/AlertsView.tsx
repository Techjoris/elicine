...
export const AlertsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="p-6 rounded-3xl bg-white dark:bg-gradient-to-r dark:from-amber-950/40 dark:via-slate-900 dark:to-slate-900 border border-slate-200 dark:border-amber-500/20 shadow-sm">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white flex items-center gap-3">
          <Bell className="w-8 h-8 text-amber-500 dark:text-amber-400" />
      ...
      {alerts.length === 0 ? (
        <div className="text-center py-20 px-4 rounded-3xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Aucune alerte active</h3>
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold uppercase shadow-sm dark:shadow-neon-gold cursor-pointer"
          >
            Explorer les films
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className="p-4 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 hover:border-amber-500/40 transition-all shadow-sm"
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white truncate">{alert.movieTitle}</h4>
